...
app.use((req, res, next) => {
  res.setHeader("Access-Control-Allow-Origin", "https://backend-clicker.onrender.com:5000");
...
app.post('/createuser', [
  ...
  body('gateways.*').isString() // Assuming gateway IDs are strings
], async (req, res) => {
...
app.get('/gateways', authenticateUser, async (req, res) => {
  try {
    const data = await User.find({ _id: req.user }); // Fetch gateways associated with the user
    // Extract the gateway objects from the array of objects
    const gateways = data.map(item => item.gateways);
    //console.log(gateways);
    return res.status(200).json(gateways);
  } catch (error) {
    console.error('Error fetching gateways:', error);
    return res.status(500).json({ error: 'Error fetching gateways.' });
  }
});
...
app.get('/data', authenticateUser, async (req, res) => {
  try {
    const gatewayId = req.header('GatewayId');
    //console.log(gatewayId);
    const data = await Click.find({ gateway_id: gatewayId });
    //console.log("data bhejdiya");
