...
const Signup = () => {
    ...
    const [numGateways, setNumGateways] = useState(0);
    const [gatewayIds, setGatewayIds] = useState([]);

    const handleSubmit = async (e) => {
        e.preventDefault();
        const response = await fetch("https://backend-clicker.onrender.com/createuser", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ name: credentials.name, email: credentials.email, password: credentials.password, geolocation: credentials.geolocation, gateways: gatewayIds })
    ...
    const handleGatewayIdChange = (e, index) => {
        const updatedGateways = [...gatewayIds];
        updatedGateways[index] = e.target.value;
        setGatewayIds(updatedGateways);
    };

    const handleNumGatewaysChange = (e) => {
        const count = parseInt(e.target.value);
        setNumGateways(count);
        setGatewayIds(new Array(count).fill(""));
    };
                    ...
                    <div className="max-w-md mx-auto">
                        ...
                        <div className="divide-y divide-gray-200">
                            <form className="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7" onSubmit={handleSubmit}>
                                ...
                                <div className="relative">
                                    <label htmlFor="numGateways" className="text-gray-600 text-sm">Number of Gateways</label>
                                    <input name="numGateways" type="number" min="0" className="peer placeholder-transparent h-10 w-full border-b-2 border-gray-300 text-gray-900 focus:outline-none focus:borer-rose-600" value={numGateways} onChange={handleNumGatewaysChange} />
                                </div>

                                {gatewayIds.map((id, index) => (
                                    <div key={index} className="relative">
                                        <input name={`gatewayId-${index}`} type="text" className="peer placeholder-transparent h-10 w-full border-b-2 border-gray-300 text-gray-900 focus:outline-none focus:borer-rose-600" placeholder={`Gateway ID ${index + 1}`} value={id} onChange={(e) => handleGatewayIdChange(e, index)} />
                                        <label htmlFor={`gatewayId-${index}`} className="absolute left-0 -top-3.5 text-gray-600 text-sm peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-440 peer-placeholder-shown:top-2 transition-all peer-focus:-top-3.5 peer-focus:text-gray-600 peer-focus:text-sm">{`Gateway ID ${index + 1}`}</label>
                                    </div>
                                ))}
